import debugpy      # 멀티 쓰레드 디버깅을 위해서 추가
...
API_KEY = "ZgOrVhe8CoBxR1N%2BDb7lvn0HVxP3q4g5Mh86RZGsthTxfS1qo3lEGInthQc1PO1QwQ5x%2Fq2AVbZbdEc9LIr2jA%3D%3D"
...
class CrawlRunnable(QRunnable):
    ...
    def crawl(self):        
        ...
        start = parse.quote(str(date_start))
        end = parse.quote(str(date_end))
        keyword = parse.quote(self.w.lineEdit.text())
        numOfRows = parse.quote(str(40))
        page = parse.quote(str(page_no))
        ...
        if(rescode==200):
            ...
            soup = BeautifulSoup(html, 'lxml')      ## html.parser사용시 지연현상 심각함 -> lxml 로 변경해도 심각
            
            totalCount = soup.find('totalcount')    # 소문자로 검색해야 함
            ...
            for itemElement in soup.find_all('item'):      ## 모두 소문자로 검색:
                #조회 데이터를 DB에 입력
